...
def process_files_decagon(files, triple_file, keeptrainone = False):
    ...
    adj_list = []
    idx_max = np.array([j for j in entity2id]).max() + 1
    for i in range(rel):
        ...
        adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (triplets_train[:, 0][idx].squeeze(1), triplets_train[:, 1][idx].squeeze(1))), shape=(idx_max, idx_max)))
    for i in range(rel, len(relation2id)):
        idx = np.argwhere(kg_triple[:, 2] == i-rel)
        # adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (kg_triple[:, 0][idx].squeeze(1), kg_triple[:, 1][idx].squeeze(1))), shape=(len(entity2id), len(entity2id))))
        adj_list.append(csc_matrix((np.ones(len(idx), dtype=np.uint8), (kg_triple[:, 0][idx].squeeze(1), kg_triple[:, 1][idx].squeeze(1))), shape=(idx_max, idx_max)))
