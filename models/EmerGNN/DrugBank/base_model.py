...
class BaseModel(object):
    ...
    def evaluate(self, test_pos, test_neg, KG):
        ...
        accuracy_for_six_class = {}
        for relation_type, relation_id in self.args.relation_class.items():
            binary_label = np.isin(label, relation_id).astype(int)
            indices = np.where(binary_label == 1)[0]
            label_new = label[indices]
            pred_new = pred[indices]
            accuracy_for_six_class[relation_type] = accuracy_score(label_new, pred_new)

        cm = confusion_matrix(label, pred, labels=range(86))
        accuracy_per_class = np.diagonal(cm) / cm.sum(axis=1)

        return f1, accuracy, kappa, accuracy_for_six_class, accuracy_per_class
    ...
    def save_model(self, out_str='', save_path=None):
        torch.save(self.model.state_dict(), save_path + '.pt')
        print(out_str, 'model saved')
