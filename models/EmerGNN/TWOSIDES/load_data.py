...
class DataLoader:
    def __init__(self, params, saved_relation2id=None):
        ...
        ddi_paths = {
            'train': os.path.join(self.task_dir, 'data/{}/{}'.format(params.sett, 'train.txt')),
            'valid': os.path.join(self.task_dir, 'data/{}/{}'.format(params.sett, 'valid_' + params.dataset + '.txt')),
            'test':  os.path.join(self.task_dir, 'data/{}/{}'.format(params.sett, 'test_' + params.dataset + '.txt')),
        }
        kg_paths = {
            'train': os.path.join(self.task_dir, 'data/{}/{}'.format(params.sett, 'relations_2hop.txt')),
            'valid': os.path.join(self.task_dir, 'data/{}/{}'.format(params.sett, 'relations_2hop.txt')),
            'test':  os.path.join(self.task_dir, 'data/{}/{}'.format(params.sett, 'relations_2hop.txt'))
        }
    ...
    def load_ent_id(self, ):
        id2entity = dict()
        id2relation = dict()
        drug_set = json.load(open('data/twosides/id2drug_em.json', 'r'))
        entity_set = json.load(open('data/twosides/entity2id_em.json', 'r'))
        relation_set = json.load(open('data/twosides/relation2id_em.json', 'r'))
    ...
    def process_files_kg(self, kg_paths, saved_relation2id=None, ratio=1):
        ...
        num_relation2id = len(self.relation2id) # ddi中有多少relation
            ...
            with open(file_path) as f:
                ...
                for triplet in file_data:
                    ...
                    if t not in self.entity2id:
                        self.entity2id[t] = t
                    # if not saved_relation2id and r-86+num_relation2id not in self.relation2id: # 将relation的id修正一下
