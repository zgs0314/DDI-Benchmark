...
class Trainer():
    ...
    def train(self):
        ...
        with open(self.params.save_result, "a") as f:
            f.write(f'best result:{self.best_result_time}:hop_{self.params.hop}_batch_{self.params.batch_size}_emb_{self.params.emb_dim}_b_{self.params.num_bases}_lr_{self.params.lr}_l2_{self.params.l2}_seed_{self.params.seed} Eval Performance: {self.best_result} Test Performance: {self.best_test_result}\n')
        if self.params.dataset != 'twosides':
            with open(self.params.save_result, "a") as f:
    ...
    def save_classifier(self):
        # torch.save(self.graph_classifier, os.path.join(self.params.exp_dir, 'checkpoints' , f'best_graph_classifier_{self.params.setting}.pth'))  # Does it overwrite or fuck with the existing file?
        torch.save(self.graph_classifier, self.params.save_pathh + '.pth')  # Does it overwrite or fuck with the existing file?
