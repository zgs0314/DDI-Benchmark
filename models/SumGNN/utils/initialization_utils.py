...
def initialize_model(params, model, load_model=False):
    ...
    # assert 0
    if load_model and os.path.exists(os.path.join(params.exp_dir, f'best_graph_classifier_{params.setting}.pth')):
        logging.info('Loading existing model from %s' % os.path.join(params.exp_dir, f'best_graph_classifier_{params.setting}.pth'))
        graph_classifier = torch.load(os.path.join(params.exp_dir, f'best_graph_classifier_{params.setting}.pth')).to(device=params.device)
    else:
